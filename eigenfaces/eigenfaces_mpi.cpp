...
auto gen_covariance(const std::vector<Vector>& z) {
  ...
  // don't normalize in this mpi version
...
auto scatter_images(const std::string& bin_file, size_t num_images, size_t height, size_t width) {
  ...
  if (myrank == 0) {
    ...
    if (mysize > 1) {
      ...
      for (size_t rank = 1; rank < mysize; ++rank) {
        for (size_t i = 0; i < sendcounts[rank]; ++i) {
          // Send z[count] out to all ranks 
          MPI_Send(&z[count](0), height*width, MPI_DOUBLE, rank, 321, MPI_COMM_WORLD);
          ++count;
        }
      }
    }
    ...
    for (size_t i = 0; i < sendcounts[myrank]; ++i) {
      // Receive myimg_data[i] from rank 0 
      MPI_Recv(&myimg_data[i](0), height*width, MPI_DOUBLE, 0, 321, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  MPI_Allreduce(&local_sum(0), &sum(0), height*width, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  MPI_Reduce(&local_C(0,0), &C(0,0), local_C.num_rows()*local_C.num_cols(), MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  ...
  if (myrank == 0) {
    ...
    if (faces_to_save > 0) {
      ...
      mean_face.save("mean_face_mpi.bmp");
