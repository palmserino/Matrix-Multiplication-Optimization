...
void outer(Matrix& A, const Vector& x) {
  // Optimization 1: Change loop ordering so that we are accessing adjacent parts of memory
  for (size_t i = 0; i < A.num_rows(); ++i) {
    double row_val = x(i); // Optimization 2: Hoist out the value that is constant along the inner loop 
